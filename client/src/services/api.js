...
export async function detectAndScript(imageBase64, targetLanguage = 'Spanish') {
  const res = await fetch(`${BASE}/detect-and-script`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ imageBase64, targetLanguage }),
  });
...
export async function checkAnswer(guess, correctObject, targetLanguage = 'Spanish') {
  const res = await fetch(`${BASE}/check-answer`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ guess, correctObject, targetLanguage }),
  });
